...
$($(PKG)_DIR)/.configured: $($(PKG)_DIR)/configure
	cd $(PERL_DIR) && PATH=$(TARGET_PATH) ./configure --host=$(GNU_HOST_NAME) --target=$(GNU_TARGET_NAME) --target-tools-prefix=$(REAL_GNU_TARGET_NAME)- --prefix=/usr $(PERL_CONFIGURE_OPTIONS) && \
	cd $(PWD) && touch $@
...
$($(PKG)_TARGET_BINARY): $($(PKG)_BINARY)
	$(SUBMAKE1) -C $(PERL_DIR) -f Makefile \
		...
		install.perl
	[ -z "$($(PKG)_VERSION)" ] || ln -f $(PERL_DEST_DIR)/usr/bin/perl$($(PKG)_VERSION) $(PERL_DEST_DIR)/usr/bin/perl
	touch $@
