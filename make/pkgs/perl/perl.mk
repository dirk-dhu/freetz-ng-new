...
ifeq ($(or $(strip $(FREETZ_OPENSSL_VERSION_1)),$(strip $(FREETZ_OPENSSL_VERSION_2))),y)
...
#$(PKG)_CONFIGURE_PRE_CMDS += $(PWD)/$(PATCH_TOOL) $(PWD)/$(PERL_DIR) $(PWD)/$(PERL_MAKE_DIR)/patches/post_configure/100-Net-SSLeay-ripemd.patch;
endif
...
ifeq ($(strip $(FREETZ_PACKAGE_PERL_DBDSQLITE)),y)
...
$(PKG)_CONFIGURE_PRE_CMDS += $(PWD)/$(PATCH_TOOL) $(PWD)/$(PERL_DIR) $(PWD)/$(PERL_MAKE_DIR)/patches/post_configure/300-DBD-SQLite_Makefile.patch;
endif
endif
...
#ifeq ($(strip $(FREETZ_PACKAGE_PERL_NETADDRESSIPLOCAL)),y)
...
ifeq ($(strip $(FREETZ_PACKAGE_PERL_HTMLPARSER)),y)
...
ifeq ($(strip $(FREETZ_PACKAGE_PERL_XMLSIMPLE)),y)
...
ifeq ($(strip $(FREETZ_PACKAGE_PERL_PODSIMPLE)),y)
...
ifeq ($(strip $(FREETZ_PACKAGE_PERL_IOSOCKETIP)),y)
...
ifeq ($(strip $(FREETZ_PACKAGE_PERL_LOCALECODES)),y)
...
ifeq ($(strip $(FREETZ_PACKAGE_PERL_XMLSAXBASE)),y)
...
ifeq ($(strip $(FREETZ_PACKAGE_PERL_XMLPARSER)),y)
...
ifeq ($(strip $(FREETZ_PACKAGE_PERL_XMLSAXEXPAT)),y)
...
ifeq ($(strip $(FREETZ_PACKAGE_PERL_XMLNAMESPACESUPPORT)),y)
...
ifeq ($(strip $(FREETZ_PACKAGE_PERL_XMLSAX)),y)
...
ifeq ($(strip $(FREETZ_PACKAGE_PERL_ENCODELOCALE)),y)
...
ifeq ($(strip $(FREETZ_PACKAGE_PERL_SCALARLISTUTILS)),y)
...
ifeq ($(strip $(FREETZ_PACKAGE_PERL_DATAUUID)),y)
...
ifeq ($(strip $(FREETZ_PACKAGE_PERL_LIBWWWPERL)),y)
...
ifeq ($(strip $(FREETZ_PACKAGE_PERL_IOHTML)),y)
...
ifeq ($(strip $(FREETZ_PACKAGE_PERL_HTTPNEGOTIATE)),y)
...
ifeq ($(strip $(FREETZ_PACKAGE_PERL_NETHTTP)),y)
...
ifeq ($(strip $(FREETZ_PACKAGE_PERL_HTTPCOOKIES)),y)
...
ifeq ($(strip $(FREETZ_PACKAGE_PERL_CRYPTRIJNDAEL)),y)
...
ifeq ($(strip $(FREETZ_PACKAGE_PERL_NETTELNET)),y)
...
ifeq ($(strip $(FREETZ_PACKAGE_PERL_HTTPMESSAGE)),y)
...
ifeq ($(strip $(FREETZ_PACKAGE_PERL_HTTPDATE)),y)
...
ifeq ($(strip $(FREETZ_PACKAGE_PERL_XMLPARSERLITE)),y)
...
ifeq ($(strip $(FREETZ_PACKAGE_PERL_CLASSINSPECTOR)),y)
...
ifeq ($(strip $(FREETZ_PACKAGE_PERL_SOAPLITE)),y)
...
ifeq ($(strip $(FREETZ_PACKAGE_PERL_LWPMEDIATYPES)),y)
...
ifeq ($(strip $(FREETZ_PACKAGE_PERL_HTTPDAEMON)),y)
...
ifeq ($(strip $(FREETZ_PACKAGE_PERL_TRYTINY)),y)
...
ifeq ($(strip $(FREETZ_PACKAGE_PERL_IOINTERFACE)),y)
...
ifeq ($(strip $(FREETZ_PACKAGE_PERL_IOSOCKETINET6)),y)
...
ifeq ($(strip $(FREETZ_PACKAGE_PERL_URIESCAPE)),y)
...
ifeq ($(strip (FREETZ_PACKAGE_PERL_CRYPTCBC)),y)
...
ifeq ($(strip (FREETZ_PACKAGE_PERL_DEVICESERIALPORT)),y)
$(PKG)_DEVICESERIALPORT_VERSION:=1.04
$(PKG)_DEVICESERIALPORT_DIR:=$(PERL_DIR)/cpan/Device-SerialPort
$(PKG)_DEVICESERIALPORT_SOURCE:=Device-SerialPort-$($(PKG)_DEVICESERIALPORT_VERSION).tar.gz
$(PKG)_DEVICESERIALPORT_SOURCE_MD5:=82c698151f934eb28c65d1838cee7d9e
$(PKG)_DEVICESERIALPORT_SITE:=http://search.cpan.org/CPAN/authors/id/C/CO/COOK
$(PKG)_DEVICESERIALPORT_CONFPATCH:=configure_DeviceSerialPort_PATCH_Makefile
$(PKG)_DEVICESERIALPORT_MAKE_ARG:="TESTPORT=/dev/ttyS1"
$(PKG)_SOURCES += $(DL_DIR)/$(PKG)_DEVICESERIALPORT_SOURCE
$(PKG)_ADDITIONAL_CPAN_MODULES += DEVICESERIALPORT
$(PKG)_CONFIGURE_PRE_CMDS += $(PWD)/$(PATCH_TOOL) $(PWD)/$(PERL_DIR) $(PWD)/$(PERL_MAKE_DIR)/patches/post_configure/200-Device-SerialPort_Makefile.patch;
endif

ifeq ($(strip (FREETZ_PACKAGE_PERL_LWPPROTOCOLHTTPS)),y)
$(PKG)_LWPPROTOCOLHTTPS_VERSION:=6.10
$(PKG)_LWPPROTOCOLHTTPS_DIR:=$(PERL_DIR)/cpan/LWP-Protocol-https
$(PKG)_LWPPROTOCOLHTTPS_SOURCE:=LWP-Protocol-https-$($(PKG)_LWPPROTOCOLHTTPS_VERSION).tar.gz
$(PKG)_LWPPROTOCOLHTTPS_SOURCE_MD5:=cf64e4bc57a9266ac4343cdf0808c5c8
$(PKG)_LWPPROTOCOLHTTPS_SITE:=http://search.cpan.org/CPAN/authors/id/O/OA/OALDERS
$(PKG)_SOURCES += $(DL_DIR)/$(PKG)_LWPPROTOCOLHTTPS_SOURCE
$(PKG)_ADDITIONAL_CPAN_MODULES += LWPPROTOCOLHTTPS
endif

ifeq ($(strip (FREETZ_PACKAGE_PERL_MOZILLACA)),y)
$(PKG)_MOZILLACA_VERSION:=20211001
$(PKG)_MOZILLACA_DIR:=$(PERL_DIR)/cpan/Mozilla-CA
$(PKG)_MOZILLACA_SOURCE:=Mozilla-CA-$($(PKG)_MOZILLACA_VERSION).tar.gz
$(PKG)_MOZILLACA_SOURCE_MD5:=843e42f367ea3c7abc5de5b6225791dc
$(PKG)_MOZILLACA_SITE:=http://search.cpan.org/CPAN/authors/id/A/AB/ABH
$(PKG)_SOURCES += $(DL_DIR)/$(PKG)_MOZILLACA_SOURCE
$(PKG)_ADDITIONAL_CPAN_MODULES += MOZILLACA
endif

ifeq ($(or $(strip $(FREETZ_OPENSSL_VERSION_1)),$(strip $(FREETZ_OPENSSL_VERSION_2))),y)
$(PKG)_DEPENDS_ON += openssl
endif
ifeq ($(strip $(FREETZ_OPENSSL_VERSION_1)),y)
$(PKG)_REBUILD_SUBOPTS += FREETZ_OPENSSL_VERSION_1
endif
ifeq ($(strip $(FREETZ_OPENSSL_VERSION_2)),y)
$(PKG)_REBUILD_SUBOPTS += FREETZ_OPENSSL_VERSION_2
endif
...
ifeq ($(strip $(FREETZ_PACKAGE_PERL_TESTPACK)),y)
$(PKG)_TESTPACK_DEP:= $($(PKG)_TESTPACK)
endif
endif
...
$(PKG)_EXTRA_LDFLAGS := -Wl,--gc-sections -Wl,-rpath=$(PERL_PREFIX)/lib/perl5/$($(PKG)_VERSION)/mips-linux/CORE
...
$($(PKG)_DIR)/configure: $($(PKG)_DIR)/.unpacked
	...
	#$(foreach mod,$(PERL_ADDITIONAL_CPAN_MODULES),[ -n "$(PERL_$(mod)_CONFPATCH)" -a -f $(PERL_DIR)/$(PERL_$(mod)_CONFPATCH) ] && cp -f $(PERL_DIR)/$(PERL_$(mod)_CONFPATCH) $(PERL_$(mod)_DIR)/configure ||:;)
	cd $(PWD); \
	$(PERL_CONFIGURE_PRE_CMDS)
	touch $@

$($(PKG)_DIR)/.configured: $($(PKG)_DIR)/configure
	cd $(PERL_DIR) && PATH=$(TARGET_PATH) ./configure --host=$(GNU_HOST_NAME) --target=$(GNU_TARGET_NAME) --target-tools-prefix=$(REAL_GNU_TARGET_NAME)- --prefix=$(PERL_PREFIX) $(PERL_CONFIGURE_OPTIONS) && \
	cd $(PWD) && touch $@
...
$($(PKG)_BINARY): $($(PKG)_DIR)/.configured $($(PKG)_DIR)/lib/lib.pm
	$(SUBMAKE1) -C $(PERL_DIR) -f Makefile \
		...
		HOST="$(GNU_HOST_NAME)" \
		OPTIMIZE="$(TARGET_CFLAGS)" \
		OPENSSL_RIPEMD_H_DIR=${FREETZ_BASE_DIR}/source/target-$(TARGET_TOOLCHAIN_COMPILER)/openssl-$(OPENSSL_VERSION)/crypto \
		DBI_INSTARCH_DIR=${FREETZ_BASE_DIR}/source/target-$(TARGET_TOOLCHAIN_COMPILER)/perl-$(PERL_VERSION)/cpan/DBI
...
$($(PKG)_TARGET_BINARY): $($(PKG)_BINARY)
	$(SUBMAKE1) -C $(PERL_DIR) -f Makefile \
		...
		install.perl
	[ -z "$($(PKG)_VERSION)" ] || ln -f $(PERL_DEST_DIR)$(PERL_PREFIX)/bin/perl$($(PKG)_VERSION) $(PERL_DEST_DIR)$(PERL_PREFIX)/bin/perl
	touch $@
...
$(pkg)-precompiled: $($(PKG)_TARGET_BINARY) $($(PKG)_TARGET_MODULES) $($(PKG)_TESTPACK_DEP)
